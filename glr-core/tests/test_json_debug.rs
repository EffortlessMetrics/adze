...
fn test_json_empty_object_debug() {
    ...
    if let Some(rules) = extracted["rules"].as_array() {
        for (i, rule) in rules.iter().enumerate() {
            ...
            if lhs == 17 && rhs_len == 2 { // object -> { }
                println!("Rule {}: Symbol {} with {} children (object -> {{ }})", i, lhs, rhs_len);
            }
            if lhs == 16 { // value rules
                println!("Rule {}: Symbol {} with {} children (value rule)", i, lhs, rhs_len);
            }
            if lhs == 15 { // document rules
                println!("Rule {}: Symbol {} with {} children (document rule)", i, lhs, rhs_len);
            }
        }
    }
