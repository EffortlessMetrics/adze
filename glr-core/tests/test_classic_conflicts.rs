...
fn test_classic_shift_reduce_conflict() {
    ...
    grammar.tokens.insert(stmt_id, Token {
        name: "stmt".to_string(),
    ...
    for state in 0..parse_table.state_count {
        for sym in 0..parse_table.symbol_count {
            if let Action::Fork(actions) = &parse_table.action_table[state][sym] {
                fork_count += 1;
                println!("\nFound Fork in state {}, symbol {}: {} actions", state, sym, actions.len());
    ...
    assert!(fork_count > 0, "Expected shift-reduce conflicts for dangling else problem, but found none");
}
...
fn test_expression_ambiguity() {
    ...
    assert!(fork_count > 0, "Expected conflicts in expression grammar without precedence");
}