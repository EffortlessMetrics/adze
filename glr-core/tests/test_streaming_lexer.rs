...
use rust_sitter_glr_core::{Driver, ParseTable, LexMode};
...
fn json_lexer(input: &str, pos: usize, _mode: LexMode) -> Option<NextToken> {
    ...
    match ch {
        b'{' => Some(NextToken { kind: 1, start: start as u32, end: (start + 1) as u32 }), // {
        b'}' => Some(NextToken { kind: 2, start: start as u32, end: (start + 1) as u32 }), // }
        b'[' => Some(NextToken { kind: 3, start: start as u32, end: (start + 1) as u32 }), // [
        b']' => Some(NextToken { kind: 4, start: start as u32, end: (start + 1) as u32 }), // ]
        b':' => Some(NextToken { kind: 5, start: start as u32, end: (start + 1) as u32 }), // :
        b',' => Some(NextToken { kind: 6, start: start as u32, end: (start + 1) as u32 }), // ,
        b'"' => {
            ...
            Some(NextToken { kind: 11, start: start as u32, end: end as u32 }) // string
        }
        b't' if bytes[p..].starts_with(b"true") => {
            Some(NextToken { kind: 8, start: start as u32, end: (start + 4) as u32 }) // true
        }
        b'f' if bytes[p..].starts_with(b"false") => {
            Some(NextToken { kind: 9, start: start as u32, end: (start + 5) as u32 }) // false
        }
        b'n' if bytes[p..].starts_with(b"null") => {
            Some(NextToken { kind: 10, start: start as u32, end: (start + 4) as u32 }) // null
        }
        b'0'..=b'9' | b'-' => {
            ...
            Some(NextToken { kind: 12, start: start as u32, end: end as u32 }) // number
        }
        _ => None
    }
}
...
fn test_streaming_json_parser() {
    ...
    let json_data = std::fs::read_to_string("/tmp/json-grammar.json")
        .expect("Failed to read JSON grammar");
