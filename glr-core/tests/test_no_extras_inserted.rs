...
fn extras_marked_correctly_in_parse_table() {
    ...
    assert!(table.is_extra(SymbolId(2)), "Symbol 2 should be marked as extra");
    ...
    assert_eq!(terminal_boundary, 3, "Terminal boundary should be token_count");
    assert!((2_usize) < terminal_boundary, "Extra symbol should be within terminal range");
...
fn external_tokens_within_insertion_range() {
    ...
    let terminal_boundary = table.terminal_boundary();
    assert_eq!(terminal_boundary, 3, "Should be token_count + external_token_count");
    
    // Test: external tokens are terminals
    assert!(table.is_terminal(SymbolId(2)), "External token should be a terminal");
