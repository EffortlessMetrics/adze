...
fn test_json_empty_object_parses() {
    ...
    let lbrace_id = symbol_id_by_name.get("{")
        .or_else(|| symbol_id_by_name.get("'{'"))
        .copied()
        .expect("Could not find '{' symbol");
    let rbrace_id = symbol_id_by_name.get("}")
        .or_else(|| symbol_id_by_name.get("'}'"))
        .copied()
        .expect("Could not find '}' symbol");
    ...
    println!("State {} on '{{' (symbol {}): {:?}", init.0, lbrace_id, actions_on_lbrace);
    
    // Find the shift target
    let after_lbrace = actions_on_lbrace.iter()
        .find_map(|a| if let Action::Shift(s) = a { Some(*s) } else { None })
    ...
    println!("State {} on '}}' (symbol {}): {:?}", after_lbrace.0, rbrace_id, actions_on_rbrace);
    
    // Assert that there IS a valid action (not empty, not just Error/Recover)
    assert!(
        !actions_on_rbrace.is_empty(),
        "State {} must have an action on '}}' for empty object", after_lbrace.0
    );
    
    // Check if it's a real parse action (Shift or Reduce), not just error recovery
    let has_valid_action = actions_on_rbrace.iter().any(|a| {
        matches!(a, Action::Shift(_) | Action::Reduce(_) | Action::Accept)
    });
    ...
    if !has_valid_action {
        ...
        println!("Actions at (state={}, symbol={}): {:?}", after_lbrace.0, rbrace_id, actions_on_rbrace);
        ...
        assert!(
            actions_on_rbrace.iter().any(|a| matches!(a, Action::Recover)),
