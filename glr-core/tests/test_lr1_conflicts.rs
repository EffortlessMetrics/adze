...
fn test_lr1_conflict_detection() {
    ...
    let has_forks = (0..parse_table.state_count).any(|state| {
        (0..parse_table.symbol_count).any(|sym| {
            matches!(&parse_table.action_table[state][sym], rust_sitter_glr_core::Action::Fork(_))
        })
    });
    
    assert!(has_forks, "Expected Fork actions in parse table for ambiguous grammar");
}