...
fn test_ambiguous_expr_lr1_construction() {
    ...
    eprintln!("  Total symbols: {:?}", grammar.rules.len() + grammar.tokens.len());
    ...
    for (idx, rule) in grammar.all_rules().enumerate() {
        let lhs_name = grammar.rule_names.get(&rule.lhs)
            .map(|s| s.as_str())
            .unwrap_or("?");

        let rhs_str: Vec<String> = rule.rhs.iter().map(|sym| {
            match sym {
...
fn test_simple_non_ambiguous_grammar() {
    ...
    match result {
        Ok(parse_table) => {
            ...
            if conflicts > 0 {
                eprintln!("  ⚠️  WARNING: Non-ambiguous grammar has {} conflicts!", conflicts);
...
fn test_left_recursive_grammar() {
    ...
    match result {
        Ok(parse_table) => {
            ...
            if conflicts == 0 {
                eprintln!("  ❌ NO CONFLICTS - but left-recursion without precedence SHOULD have conflicts!");
