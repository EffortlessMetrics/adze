...
fn test_eof_not_error_symbol() {
    ...
    println!("✓ Driver created successfully with EOF={} (not 0)", parse_table.eof_symbol.0);
}
...
fn test_error_stats_not_stubbed() {
    ...
    let mut parse_table = ParseTable {
        ...
        rules: vec![
            ParseRule { lhs: SymbolId(10), rhs_len: 0 },  // start -> ε
            ParseRule { lhs: SymbolId(10), rhs_len: 2 },  // start -> { }
        ],
        state_count: 3,
        symbol_count: 11,
        symbol_to_index: BTreeMap::new(),
    ...
    match result {
        Ok(forest) => {
            ...
            assert!(cost > 0, "Expected non-zero cost from recovery, got {}", cost);
            
            println!("✓ Error stats correctly reported: has_error={}, missing={}, cost={}",
                has_error, missing, cost);
        }
        Err(e) => {
