// GLR version tracking for conflict resolution
// This module implements Tree-sitter's ts_parser__compare_versions logic
...
pub struct VersionInfo {
    ...
    /// Sum of all dynamic precedence values in the parse tree
...
pub enum CompareResult {
    ...
    PreferRight,
    /// They are equivalent (need symbol comparison)
    Tie,
}

/// Compare two parse versions according to Tree-sitter's exact algorithm
/// Port of ts_parser__compare_versions from parser.c
pub fn compare_versions(a: &VersionInfo, b: &VersionInfo) -> CompareResult {
    ...
    if a.cost != b.cost {
        ...
        let threshold = MAX_COST_DIFF_FACTOR * ERROR_COST_PER_SKIPPED_TREE * total_node_count.max(1);
        ...
        if cost_diff >= threshold {
            ...
            // Small cost difference - prefer but don't take
            return if a.cost < b.cost {
    ...
    // Step 4: Fallback to symbol comparison (handled elsewhere)
    CompareResult::Tie
}
...
mod tests {
    ...
    fn test_cumulative_dynamic_precedence() {
        ...
        // Simulate multiple reductions with dynamic precedence
