...
impl GrammarJsParserV3 {
    ...
    fn parse_function_block(&self, block: &str) -> Result<Rule> {
        ...
        // Keep JavaScript block concerns in the function_block module, then parse
        // the extracted grammar rule expression here.
        eprintln!("Debug: parse_function_block called with block:\n{}", block);

        match super::function_block::extract_return_rule_expression(block)? {
            Some(return_expr) => self.parse_rule(&return_expr),
            None => Ok(Rule::Choice { members: vec![] }),
        }
    }
