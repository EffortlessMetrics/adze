...
//! rust-sitter's generate_grammars to the GrammarJs intermediate format.
...
fn parse_rule(value: &Value) -> Result<Rule> {
    let obj = value.as_object()
        .context("Expected object for rule")?;
    ...
    match rule_type {
        "STRING" => {
            ...
            Ok(Rule::String { value: value.to_string() })
        }
        ...
        "PATTERN" => {
            ...
            Ok(Rule::Pattern { value: value.to_string() })
        }
        ...
        "SYMBOL" => {
            ...
            Ok(Rule::Symbol { name: name.to_string() })
        }
        ...
        "PREC" => {
            ...
            let content = obj.get("content")
                .context("PREC rule missing 'content'")?;
            Ok(Rule::Prec { 
                value: value as i32,
                content: Box::new(parse_rule(content)?) 
            })
        ...
        "TOKEN" => {
            let content = obj.get("content")
                .context("TOKEN rule missing 'content'")?;
            Ok(Rule::Token { 
                content: Box::new(parse_rule(content)?) 
            })
        }
        ...
        "FIELD" => {
            ...
            let content = obj.get("content")
                .context("FIELD rule missing 'content'")?;
            Ok(Rule::Field { 
                name: name.to_string(),
                content: Box::new(parse_rule(content)?) 
            })
        ...
        "ALIAS" => {
            ...
            let named = obj.get("named")
                ...
                .unwrap_or(false);
            let content = obj.get("content")
                .context("ALIAS rule missing 'content'")?;
            Ok(Rule::Alias { 
                content: Box::new(parse_rule(content)?),
