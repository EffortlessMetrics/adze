...
pub fn build_parser(mut grammar: Grammar, options: BuildOptions) -> Result<BuildResult> {
    ...
    if options.emit_artifacts {
        ...
        #[cfg(feature = "serialization")]
        {
            ...
            let writer =
                ParsetableWriter::new(&grammar, &parse_table, &grammar_name, &grammar_version);
...
module.exports = grammar({
  ...
  rules: {
    ...
    fn test_desugar_pattern_wrappers_structural() {
        ...
        let rules = grammar.rules.get(&nt_id);
        assert!(rules.is_some(), "Should have added rules for Identifier using structural matching");
        let rules = rules.unwrap();
        assert_eq!(rules.len(), 1);
        assert_eq!(rules[0].rhs.len(), 1);
        match &rules[0].rhs[0] {
