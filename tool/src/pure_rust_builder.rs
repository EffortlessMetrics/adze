...
pub fn build_parser(mut grammar: Grammar, options: BuildOptions) -> Result<BuildResult> {
    ...
    writeln!(
        parser_file,
        "#[allow(unused_unsafe, unused_variables, unexpected_cfgs, unsafe_op_in_unsafe_fn)]"
    )?;
    writeln!(parser_file, "#[allow(clippy::all)]")?;
