...
use rust_sitter_ir::{Grammar, Symbol, SymbolId, Rule};
...
impl GrammarVisualizer {
    ...
    pub fn to_dot(&self) -> String {
        ...
        for (id, token) in &self.grammar.tokens {
            ...
            writeln!(&mut output, "  t{} [label=\"{}\" shape=ellipse style=filled fillcolor=lightblue];", 
                id.0, label).unwrap();
        }
        ...
        for id in self.grammar.rules.keys() {
            ...
            writeln!(&mut output, "  n{} [label=\"{}\" style=filled fillcolor=lightgreen];", 
                id.0, self.escape_dot(&name)).unwrap();
        }
        
        writeln!(&mut output, "\n  // External tokens").unwrap();
        for external in &self.grammar.externals {
            writeln!(&mut output, "  e{} [label=\"{}\" shape=diamond style=filled fillcolor=lightcoral];", 
                external.symbol_id.0, self.escape_dot(&external.name)).unwrap();
        }
    ...
    pub fn to_railroad_svg(&self) -> String {
        ...
        writeln!(&mut output, r#"<svg xmlns="http://www.w3.org/2000/svg" width="{}" height="600">"#, width).unwrap();
        writeln!(&mut output, r#"  <style>"#).unwrap();
        writeln!(&mut output, r#"    .rule-name {{ font-family: monospace; font-weight: bold; }}"#).unwrap();
        writeln!(&mut output, r#"    .terminal {{ fill: #4a90e2; }}"#).unwrap();
        writeln!(&mut output, r#"    .non-terminal {{ fill: #7ed321; }}"#).unwrap();
        writeln!(&mut output, r#"    .line {{ stroke: #333; stroke-width: 2; fill: none; }}"#).unwrap();
        writeln!(&mut output, r#"  </style>"#).unwrap();
            ...
            for symbol in &rule.rhs {
                let (text, class) = match symbol {
                    ...
                    Symbol::NonTerminal(id) => {
                        (self.get_symbol_name(*id), "non-terminal")
                    }
                    Symbol::External(id) => {
                ...
                // Draw text
                writeln!(&mut output, r#"  <text x="{}" y="{}" text-anchor="middle">{}</text>"#, 
                    x_offset + text_width / 2, y_offset + 5, self.escape_xml(&text)).unwrap();
                
                // Draw connecting line
                if x_offset > 150 {
                    writeln!(&mut output, r#"  <line x1="{}" y1="{}" x2="{}" y2="{}" class="line"/>"#, 
                        x_offset - 10, y_offset, x_offset, y_offset).unwrap();
                }
    ...
    pub fn to_text(&self) -> String {
        ...
        if !self.grammar.externals.is_empty() {
            writeln!(&mut output, "\nExternal Tokens:").unwrap();
            for external in &self.grammar.externals {
                writeln!(&mut output, "  {} ({:?})", external.name, external.symbol_id).unwrap();
            }
        }
...
mod tests {
    use super::*;
    use rust_sitter_ir::{Token, TokenPattern, ProductionId};
    ...
    fn test_grammar_to_text() {
        ...
        grammar.tokens.insert(id_sym, Token {
