...
fn test_javascript_grammar_parsing() {
    ...
    let grammar_url = "https://raw.githubusercontent.com/tree-sitter/tree-sitter-javascript/master/grammar.js";
    ...
    if !output.status.success() {
        panic!("Failed to download JavaScript grammar: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    match build_result {
        Ok(build_result) => {
            ...
            if node_types_path.exists() {
                ...
                let node_names: Vec<&str> = json.as_array().unwrap()
                    .iter()
                    .filter_map(|n| n["type"].as_str())
                    .collect();
                
                println!("Found {} node types", node_names.len());
                println!("Sample node types: {:?}", &node_names[..10.min(node_names.len())]);
                
                // Check for some expected JavaScript node types
                let expected_types = [
                    "program", "statement_block", "function_declaration", 
                    "variable_declaration", "identifier", "binary_expression"
                ];
                ...
                assert!(found_count >= 3, "Should find at least 3 of the expected types, found {}", found_count);
                assert!(node_names.len() > 50, "JavaScript grammar should have many node types, found {}", node_names.len());
            }
        }
        Err(e) => {
