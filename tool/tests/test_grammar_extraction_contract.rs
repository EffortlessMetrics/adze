    ...
    fn print_summary(&self) {
        ...
        for (idx, pattern) in self.production_patterns.iter().enumerate() {
            eprintln!(
                ...
                pattern.lhs,
                pattern.rhs_count,
                if pattern.is_left_recursive {
                    " [LEFT-RECURSIVE]"
                } else {
                    ""
                },
                if pattern.has_precedence {
                    " [HAS-PREC]"
                } else {
                    ""
                }
...
fn test_contract_manual_grammar_structure() {
    ...
    let has_recursive_pattern = analysis
        .production_patterns
        .iter()
        .any(|p| p.lhs == "binary" && p.rhs_count == 3);

    assert!(
        has_recursive_pattern,
    ...
    assert_eq!(
        analysis.count_productions_with_rhs_len(3),
        1,
    ...
    let has_any_precedence = analysis
        .production_patterns
        .iter()
        .any(|p| p.has_precedence);
    assert!(
        !has_any_precedence,
...
fn test_contract_enum_grammar_extraction() {
    ...
    let grammars =
        generate_grammars(&example_path).expect("Failed to extract grammar from ambiguous_expr.rs");
    ...
    for (rule_name, rule_def) in rules {
        ...
        eprintln!(
            "  Definition: {}",
            serde_json::to_string_pretty(rule_def).unwrap()
        );
    }
...
fn test_contract_production_pattern_comparison() {
    ...
    assert_eq!(
        analysis1.total_rules, analysis2.total_rules,
        "Rule count must be independent of definition order"
    );
    assert_eq!(
        analysis1.total_terminals, analysis2.total_terminals,
        "Terminal count must be independent of definition order"
    );
    assert_eq!(
        analysis1.has_left_recursive_production(),
        analysis2.has_left_recursive_production(),
...
fn test_contract_recursion_preservation() {
    ...
    assert!(
        recursive_analysis.has_left_recursive_production(),
        "Contract violation: Left-recursive grammar not detected"
    );
    assert!(
        !non_recursive_analysis.has_left_recursive_production(),
...
fn test_contract_symbol_naming() {
    ...
    assert!(
        analysis.symbol_names.contains("NUMBER"),
        "Terminal symbols must use provided names"
    );
    assert!(
        analysis.symbol_names.contains("expr"),
        "Non-terminal names must be preserved"
    );
    assert!(
        analysis.symbol_names.contains("term"),
        "All non-terminals must be named"
    );

    // Contract: No auto-generated complex names like "Expr_Binary_Expr_Binary_1"
    let has_complex_names = analysis
        .symbol_names
        .iter()
...
fn test_contract_precedence_handling() {
    ...
    assert!(
        prec_count > 0,
        "Grammar with precedence must have precedence in productions"
    );
    eprintln!("✅ Productions with precedence: {}", prec_count);

    let no_prec_count = without_analysis
        .production_patterns
        .iter()
        .filter(|p| p.has_precedence)
        .count();
    ...
    assert_eq!(
        no_prec_count, 0,
        "Grammar without precedence must not have precedence in productions"
    );
...
fn test_inlined_enum_matches_manual_grammar() {
    ...
    let grammars = generate_grammars(&example_path).expect("Failed to extract grammar");
...
fn test_precedence_prevents_inlining() {
    ...
    let grammars =
        generate_grammars(&arithmetic_path).expect("Failed to extract arithmetic grammar");
    ...
    let has_intermediates = rules
        .keys()
        .any(|k| k.starts_with("Expr_") && k != "Expr" && !k.ends_with("_unit"));
...
fn test_inlining_preserves_field_structure() {
    ...
    eprintln!(
        "✅ Fields named with variant context (e.g., Expr_Binary_0, Expr_Binary_1, Expr_Binary_2)"
    );
}
...
fn test_contract_enum_transformation_spec() {
    ...
    eprintln!("        Expr: [");
    eprintln!(
        "            Rule {{ lhs: Expr, rhs: [NonTerminal(Expr), Terminal(OP), NonTerminal(Expr)], ... }},"
    );
