//! Build-system utilities for discovering and integrating external scanners.
...
pub enum ScannerLanguage {
    /// C scanner implementation (`.c`).
    C,
    /// C++ scanner implementation (`.cc` or `.cpp`).
    Cpp,
    /// Rust scanner implementation (`.rs`).
    Rust,
}
...
impl ScannerBuilder {
    ...
    pub fn build(&self) -> Result<()> {
        ...
        match scanner.language {
            ScannerLanguage::C | ScannerLanguage::Cpp => self.build_c_scanner(&scanner)?,
            ScannerLanguage::Rust => self.build_rust_scanner(&scanner)?,
        }
