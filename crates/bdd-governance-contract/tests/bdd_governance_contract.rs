//! BDD-style tests for bdd-governance-contract crate.
//!
...
#[test]
fn given_profile_with_glr_when_describing_backend_then_returns_glr_name() {
    // Given
    ...
    assert_eq!(desc, ParserBackend::GLR.name());
}
...
#[test]
fn given_matrix_when_calling_report_then_report_contains_summary_header() {
    // Given
    let matrix = BddGovernanceMatrix::standard(ParserFeatureProfile::current());

    // When
    let report = matrix.report("Test Phase");
