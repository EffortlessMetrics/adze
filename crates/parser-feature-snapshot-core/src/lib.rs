//! Snapshot of parser feature flags for metadata and diagnostics.
...
mod tests {
    ...
    fn round_trip_profile() {
        ...
        assert_eq!(
            ParserFeatureProfileSnapshot::from_profile(profile).as_profile(),
            profile
        );
    }

    #[test]
    fn non_conflict_backend_prefers_glr_then_pure_rust() {
        assert_eq!(
            ParserFeatureProfileSnapshot::new(true, true, true, true).non_conflict_backend(),
            ParserBackend::GLR.name()
        );
        assert_eq!(
            ParserFeatureProfileSnapshot::new(true, true, true, false).non_conflict_backend(),
            ParserBackend::PureRust.name()
        );
        assert_eq!(
            ParserFeatureProfileSnapshot::new(false, true, true, false).non_conflict_backend(),
            ParserBackend::TreeSitter.name()
        );
    }
}
