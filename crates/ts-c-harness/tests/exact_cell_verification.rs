...
fn verify_exact_cell_after_lbrace() {
    ...
    if !Path::new(tables_path).exists() {
        ...
        if !output.status.success() {
            panic!("Failed to build ts-bridge: {}", String::from_utf8_lossy(&output.stderr));
        }
        
        // Extract tables
        let output = Command::new("cargo")
            .args(&["run", "-p", "ts-bridge", "--features", "with-grammars", "--", 
                   "--grammar", "json", 
                   "--output", tables_path])
            .output()
    ...
    if let Some(actions) = lbrace_actions {
        ...
        let shift_state = actions.as_array()
...
fn verify_runtime_behavior_matches_extraction() {
    ...
    for (input, should_error, desc) in test_cases {
        ...
        assert_eq!(has_error, should_error, 
            "Mismatch for {}: expected error={}, got={}", desc, should_error, has_error);
    }
}