...
fn check_json_tables_directly() {
    ...
    let json_str = fs::read_to_string(output_path).expect("Failed to read extracted tables");

    let tables: serde_json::Value = serde_json::from_str(&json_str).expect("Failed to parse JSON");

    // Get symbol list
    let symbols = tables["symbols"].as_array().expect("No symbols array");
...
fn verify_json_language_loads() {
    ...
    assert!(
        ...
        "Empty object should parse without error"
    );
