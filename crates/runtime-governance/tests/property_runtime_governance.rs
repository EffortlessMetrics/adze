//! Property-based tests for runtime-governance.
...
// ---------------------------------------------------------------------------
// 3 – Report function tests
// ---------------------------------------------------------------------------

proptest! {
    #[test]
    fn bdd_progress_report_for_current_profile_non_empty(phase in arb_phase()) {
        let report = adze_runtime_governance::bdd_progress_report_for_current_profile(phase, "Test");
        prop_assert!(!report.is_empty());
    }

    #[test]
    fn bdd_status_line_for_current_profile_non_empty(phase in arb_phase()) {
        let line = adze_runtime_governance::bdd_status_line_for_current_profile(phase);
