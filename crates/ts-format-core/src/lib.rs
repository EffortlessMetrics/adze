...
mod tests {
    ...
    #[test]
    fn choose_action_prefers_accept_then_shift() {
        let shift = Action::Shift(StateId(7));
        let accept = Action::Accept;

        assert_eq!(choose_action(&[Action::Error, shift.clone()]), Some(shift));
        assert_eq!(
            choose_action(&[Action::Error, accept.clone()]),
            Some(accept)
        );
    ...
    #[test]
    fn precedence_aware_selector_prefers_accept() {
        let table = ParseTable::default();

        let selected =
            choose_action_with_precedence(&[Action::Shift(StateId(2)), Action::Accept], &table);
        assert_eq!(selected, Some(Action::Accept));
    }
}
