use adze_concurrency_init_rayon_core::classifier::is_already_initialized_error as classifier_module_fn;
...
fn rayon_init_core_reexport_matches_classifier_core_behavior() {
    for message in [
        ...
        assert_eq!(rayon_core_fn(message), classifier_module_fn(message));
    }
}
...
fn rayon_init_core_reexport_is_type_compatible_with_classifier_core() {
    ...
    assert_eq!(
        ...
        classifier_module_fn("The global thread pool has already been initialized")
    );
}
