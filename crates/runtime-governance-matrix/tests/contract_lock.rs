...
//! BDD reporting wiring around the canonical GLR preservation grid.

use adze_runtime_governance_matrix::{
    BddGovernanceMatrix, BddGovernanceSnapshot, BddPhase, BddScenario, BddScenarioStatus,
    GLR_CONFLICT_FALLBACK, GLR_CONFLICT_PRESERVATION_GRID, ParserBackend,
    ...
    bdd_progress_report, bdd_progress_report_for_current_profile,
    ...
    runtime_governance_snapshot, runtime2_governance_snapshot,
};
...
fn test_contract_lock_functions() {
    ...
    let _profile2 = parser_feature_profile_for_runtime2(true);
...
fn test_contract_lock_snapshot_functions() {
    ...
    let snapshot2 = runtime2_governance_snapshot(BddPhase::Runtime, profile);
    assert_eq!(snapshot2.phase, BddPhase::Runtime);
}
