//! BDD-style tests for parser-governance-contract crate.
//!
//! Tests follow the Given/When/Then pattern to verify public API behavior.

use adze_parser_governance_contract::{
    BddGovernanceMatrix, BddPhase, BddScenarioStatus, GLR_CONFLICT_FALLBACK,
    GLR_CONFLICT_PRESERVATION_GRID, ParserBackend, ParserFeatureProfile, bdd_progress,
...
fn given_title_when_calling_bdd_progress_report_then_report_contains_title() {
    // Given
    let title = "Governance Contract Test Report";
...
#[test]
fn given_glr_fallback_grid_when_checking_is_not_empty_then_returns_true() {
    // Given / When
    let is_empty = GLR_CONFLICT_FALLBACK.is_empty();
...
#[test]
fn given_preservation_grid_when_checking_is_not_empty_then_returns_true() {
    // Given / When
    let is_empty = GLR_CONFLICT_PRESERVATION_GRID.is_empty();
...
#[test]
fn given_profile_with_glr_when_calling_has_glr_then_returns_boolean() {
    // Given
    let profile = ParserFeatureProfile::current();
