...
/// Tree-sitter symbol identifier type.
...
pub struct ParsedNode {
    ...
    /// Byte range end in source text.
    pub end_byte: usize,
    /// Position in lines/columns.
    pub start_point: Point,
    /// End position in lines/columns.
    pub end_point: Point,
    /// Node flags.
    pub is_extra: bool,
    /// Node flags.
...
impl ParsedNode {
    ...
    pub fn from_reduction(
        ...
        let (start_byte, end_byte) = if parsed_children.is_empty() {
            ...
            let end = parsed_children.last().expect("non-empty").node.end_byte;
            (start, end)
        };
        ...
        let (start_point, end_point) = if parsed_children.is_empty() {
            ...
            let end = parsed_children.last().expect("non-empty").node.end_point;
            (start, end)
        };
