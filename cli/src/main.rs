...
fn parse_file_dynamic(
    ...
    unsafe {
        ...
        let get_language: libloading::Symbol<unsafe extern "C" fn() -> *const u8> =
            lib.get(&sym_name)?;
        let lang_ptr = get_language() as *const rust_sitter::pure_parser::TSLanguage;
        let language: &'static rust_sitter::pure_parser::TSLanguage = &*lang_ptr;
        ...
        if let Some(root) = result.root {
            match format {
                OutputFormat::Json => {
                    // JSON output would require serialization support
                    println!(
                        "{{\"status\": \"parsing successful\", \"nodes\": \"{}\"}}",
                        node_to_sexp(&root, &input_content, 0).replace('"', "\\\"")
                    );
                }
                OutputFormat::Sexp => {
                    println!("{}", node_to_sexp(&root, &input_content, 0));
                }
                _ => {
                    println!("{}", node_to_sexp(&root, &input_content, 0));
                }
