...
fn main() -> anyhow::Result<()> {
    ...
    if args.len() < 2 {
        eprintln!(
            "Usage: {} <path/to/libtree-sitter-*.so> [output.json] [symbol_name]",
            args[0]
        );
        eprintln!(
            "Example: {} libtree-sitter-json.so parse_table.json tree_sitter_json",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    let lib = unsafe {
        Library::new(path).map_err(|e| anyhow::anyhow!("Failed to load library {}: {}", path, e))?
    };
    ...
    eprintln!(
        "  - start symbol: {} ('{}')",
        data.start_symbol, &data.symbols[data.start_symbol as usize].name
    );
