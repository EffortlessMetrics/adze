...
fn debug_extraction_state_0() {
    ...
    use ts_bridge::ffi::{SafeLang, TSLanguage};
    ...
    for sym in 0..15 {
        if let Some((hdr, idx)) = lang.entry(0, sym) {
            println!(
                "State 0, symbol {} ({}): {} actions at index {}",
                sym,
                lang.symbol_name(sym),
                hdr.count,
                idx
            );
    ...
    for sym in 0..15 {
        if let Some((hdr, idx)) = lang.entry(1, sym) {
            ...
            println!(
                "State 1, symbol {} ({}): {} actions at index {}",
                ...
                match action.kind {
                    ...
                    ts_bridge::ffi::TsbActionKind::Reduce => {
                        println!(
                            "  -> Reduce: lhs={} ({}), rhs_len={}",
                            action.lhs,
                            lang.symbol_name(action.lhs as u32),
                            action.rhs_len
                        );
                    }
                    _ => {
