...
use rust_sitter_testing::{TestConfig, TestSuite, BetaTester};
...
fn test_single_grammar(
    ...
    if result.speedup > 0.0 {
        println!("  Performance: {:.2}x faster than tree-sitter", result.speedup);
    }
...
fn run_test_suite(config_path: PathBuf, output_dir: PathBuf) -> Result<()> {
    ...
    println!("\nTest suite complete!");
    println!("  Overall compatibility: {:.2}%", report.overall_compatibility);
...
fn test_corpus(
    ...
    for grammar_info in grammars_to_test {
        ...
        let test_config = TestConfig {
            ...
            compare_output: true,
            benchmark: true,
            external_scanner: grammar_info.has_scanner.then_some(grammar_info.name.clone()),
        };
...
fn find_corpus_grammars(tree_sitter_path: &PathBuf) -> Result<Vec<CorpusGrammarInfo>> {
    ...
    for dir in grammar_dirs {
        ...
        for entry in std::fs::read_dir(&dir)? {
            ...
            let has_scanner = path.join("src/scanner.c").exists() ||
                             path.join("src/scanner.cc").exists();
