...
use rust_sitter_tablegen::{TableCompressor, CompressedTables};
...
fn test_precedence_handling() {
    ...
    assert!(token_indices.len() >= 6, "Should have at least 6 token indices");
}
...
fn test_recursive_grammar() {
    ...
    assert!(!compressed.action_table.is_empty());
    assert!(compressed.action_table.len() >= 3, "Should have at least 3 actions");
}