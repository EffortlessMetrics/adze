...
use rust_sitter_ir::{Grammar, Rule, Symbol, SymbolId, Token, TokenPattern, ProductionId, FieldId, PrecedenceKind, StateId, RuleId};
use rust_sitter_tablegen::{StaticLanguageGenerator, TableCompressor, NodeTypesGenerator};
...
fn test_node_types_generation() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(&node_types)
...
fn test_compressed_table_format() {
    ...
    assert_eq!(compressed.action_table.row_offsets.len(), parse_table.state_count + 1);
    assert_eq!(compressed.action_table.default_actions.len(), parse_table.state_count);
    
    // Check goto table structure
    assert_eq!(compressed.goto_table.row_offsets.len(), parse_table.state_count + 1);
}
