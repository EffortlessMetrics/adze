...
fn test_goto_table_compression() {
    ...
    let goto_table = vec![
        ...
        vec![StateId(2), StateId(2), StateId(3), StateId(3), StateId(3), StateId(3)], // Runs of 2 and 4
    ];
...
fn test_deterministic_table_generation() {
    ...
    let compressor = TableCompressor::new();
    let compressed1 = compressor.compress(&parse_table1).unwrap();
    let compressed2 = compressor.compress(&parse_table2).unwrap();
    
    // Verify deterministic compression
    assert_eq!(compressed1.action_table.data.len(), compressed2.action_table.data.len());
    assert_eq!(compressed1.goto_table.data.len(), compressed2.goto_table.data.len());
}
