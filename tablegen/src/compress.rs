    ...
    pub fn compress(
        ...
        if let Some(state0_actions) = parse_table.action_table.first() {
            ...
            if !has_token_shift && !eof_ok {
                ...
                #[expect(clippy::needless_range_loop, reason = "idx is used to index into state0_actions directly; the range loop is clearer than enumerate for this debug-info path")]
