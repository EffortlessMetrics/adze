...
|---|---|---|---|---|
| Typed extraction | **Stable** | `cargo test -p adze --lib --tests --bins` (via `just ci-supported`); product canary: `cargo test -p adze --features pure-rust --test typed_ast_contract typed_ast_contract_left_associative_addition -- --exact --nocapture` | `CI / ci-supported`; product-proof advisory | Core user contract via `adze` runtime tests in the supported lane, with an exact-value downstream canary in the advisory product lane. |
| Pure-Rust parser | **Stable** | `just ci-supported`; product canary: `cargo test -p adze --features pure-rust --test typed_ast_contract typed_ast_contract_left_associative_addition -- --exact --nocapture` | `CI / ci-supported`; product-proof advisory | Supported gate exercises pure-Rust core crates as the required contract; advisory canary proves generated pure-Rust parsing into a typed value. |
| Operator precedence | **Stable** | `cargo test -p adze --features pure-rust --test typed_ast_contract typed_ast_contract_left_associative_addition -- --exact --nocapture`; `cargo test -p adze-glr-core test_precedence_resolves_add_mul -- --exact` | `CI / ci-supported`; product-proof advisory | Stable for proven expression grammar shapes, not a blanket claim for every ambiguous grammar. |
