...
| GLR conflict routing | **Stabilizing** | `cargo test -p adze-glr-core conflict ambiguity -- --nocapture`; product canary: `cargo test -p adze --features "pure-rust,glr,runtime-e2e" --test test_e2e_ambiguous_grammar_glr test_ambiguous_grammar_glr_parsing -- --exact --nocapture`; target: `cargo test -p adze --features "pure-rust,glr,runtime-e2e" glr_ -- --nocapture` | `CI / ci-supported`; product-proof advisory | GLR core is in the required gate and one ambiguous typed-extraction path is product-canary gated, but full product proof still needs broader conflict-preserving E2E coverage plus ambiguity metadata/selection coverage. |
