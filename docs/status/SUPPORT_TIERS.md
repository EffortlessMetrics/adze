...
| GLR conflict routing | **Stabilizing** | `cargo test -p adze-glr-core conflict ambiguity -- --nocapture`; product canaries: `cargo test -p adze --features "pure-rust,glr,runtime-e2e" --test test_e2e_ambiguous_grammar_glr test_ambiguous_grammar_conflict_generation -- --exact --nocapture`, `cargo test -p adze --features "pure-rust,glr,runtime-e2e" --test test_e2e_ambiguous_grammar_glr test_ambiguous_grammar_glr_parsing -- --exact --nocapture`; target: `cargo test -p adze --features "pure-rust,glr,runtime-e2e" glr_ -- --nocapture` | `CI / ci-supported`; product-proof advisory | GLR core is in the required gate and advisory canaries now cover generated conflict-cell preservation plus one ambiguous typed-extraction path, but full product proof still needs broader E2E conflict coverage plus ambiguity metadata/selection coverage. |
| Tablegen `TSLanguage` ABI | **Stabilizing** | `cargo test -p adze-tablegen --all-features`; adjacent product canary: `cargo test -p adze --features "pure-rust,glr,runtime-e2e" --test test_e2e_ambiguous_grammar_glr test_ambiguous_grammar_conflict_generation -- --exact --nocapture`; target: `cargo test -p adze --features "pure-rust,glr,ts-compat" --test ts_compat_equiv -- --nocapture` | `CI / ci-supported`; product-proof advisory; future product lane | Symbol/state invariants are core-gated and one advisory canary proves generated ambiguous tables retain multi-action conflict cells; full conflict encoding/routing, roundtrip semantics, and real field maps still need stronger ABI proof before broader Tree-sitter compatibility is claimed. |
| Structured parse errors | **Stabilizing** | product canary: `cargo test -p adze --test error_display_tests reporting_parse_with_errors_includes_source_excerpt_after_bad_input --features "pure-rust,glr" -- --exact --nocapture`; target: `cargo test -p adze --features "pure-rust,glr" parse_error -- --nocapture` | Product-proof advisory; future stable product lane | Error paths have an advisory canary for expected sets plus source excerpts, but the stable contract still needs broader span, line/column, excerpt, no-panic, LR/GLR parity coverage. |
| Serialization | **Stable for core table serialization** | `cargo test -p adze-glr-core --features serialization --doc` (via `just ci-supported`); product canary: `cargo test -p adze-glr-core --features serialization --test serialization_v9 sv9_complex_precedence_roundtrip -- --exact --nocapture` | `CI / ci-supported`; product-proof advisory | Supported proof is core `ParseTable` serialization, with an advisory generated-precedence grammar roundtrip canary. Runtime tree JSON/S-expression helpers have tests, but they are not the stable README product claim until promoted explicitly. |
