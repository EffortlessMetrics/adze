...
- `adze-golden-tests` JavaScript canary — **behavior** (`cargo test -p adze-golden-tests javascript_canary_expression_golden --features javascript-grammar -- --ignored --nocapture`) — currently `#[ignore]` due to a JavaScript grammar regression that prevents parsing a bare `;` as `EmptyStatement` (introduced between PR #456 and main). Re-enable by dropping the `#[ignore]` once the grammar parses the canary fixture again.
