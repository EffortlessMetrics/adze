...
use glr_query::{QueryParser, QueryCursor, Subtree};
...
fn create_json_grammar() -> Grammar {
    ...
    // value → object
    grammar.rules.insert(SymbolId(30), Rule {
        lhs: value_id,
    ...
    // value → array
    grammar.rules.insert(SymbolId(31), Rule {
        lhs: value_id,
    ...
    // value → string
    grammar.rules.insert(SymbolId(32), Rule {
        lhs: value_id,
    ...
    // value → number
    grammar.rules.insert(SymbolId(33), Rule {
        lhs: value_id,
    ...
    // value → true
    grammar.rules.insert(SymbolId(34), Rule {
        lhs: value_id,
    ...
    // value → false
    grammar.rules.insert(SymbolId(35), Rule {
        lhs: value_id,
    ...
    // value → null
    grammar.rules.insert(SymbolId(36), Rule {
        lhs: value_id,
    ...
    // array → [ ]
    grammar.rules.insert(array_id, Rule {
        lhs: array_id,
    ...
    // pair → string : value
    grammar.rules.insert(pair_id, Rule {
        lhs: pair_id,
        rhs: vec![Symbol::Terminal(string_id), Symbol::Terminal(colon_id), Symbol::NonTerminal(value_id)],
        precedence: None,
        associativity: None,
...
fn run_queries(grammar: &Grammar, tree: &Subtree) {
    ...
    let query = QueryParser::new(grammar, "(pair (string) @key (colon) (value) @value)").parse().unwrap();
    let cursor = QueryCursor::new();
    let matches: Vec<_> = cursor.matches(&query, tree).collect();
    ...
    let query = QueryParser::new(grammar, "(string) @str (number) @num").parse().unwrap();
    let cursor = QueryCursor::new();
    let matches: Vec<_> = cursor.matches(&query, tree).collect();
    ...
    println!("   Found {} string(s) and {} number(s)", string_matches, number_matches);
}
...
fn demo_advanced_queries() {
    ...
    let example_queries = vec![
        ...
        ("(pair (string) @key (colon) (value) @val)", "Capture key-value pairs"),
        ...
        ("(pair (string) @k1) (pair (string) @k2) (#eq? @k1 @k2)", "Pairs with equal keys"),
    ];
