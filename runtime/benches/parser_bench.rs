...
use rust_sitter_ir::{Grammar, Rule, RuleId, StateId, Symbol, SymbolId, Token as IrToken, TokenPattern, ProductionId};
...
fn create_arithmetic_grammar() -> Grammar {
    ...
    grammar.skip_tokens.insert(SymbolId(6), IrToken {
    ...
    grammar.non_terminals.insert(SymbolId(10), "expression".to_string());
    grammar.non_terminals.insert(SymbolId(11), "term".to_string());
    grammar.non_terminals.insert(SymbolId(12), "factor".to_string());
    ...
    // term -> term * factor
    grammar.rules.insert(RuleId(2), Rule {
        lhs: SymbolId(11),
        rhs: vec![
            Symbol::NonTerminal(SymbolId(11)),
    ...
    // term -> factor
    grammar.rules.insert(RuleId(3), Rule {
        lhs: SymbolId(11),
    ...
    // factor -> number
    grammar.rules.insert(RuleId(4), Rule {
        lhs: SymbolId(12),
    ...
    // factor -> ( expression )
    grammar.rules.insert(RuleId(5), Rule {
        lhs: SymbolId(12),
        rhs: vec![
...
fn create_parse_table() -> ParseTable {
    ...
    // Add symbol metadata
    table.symbol_metadata.insert(SymbolId(1), SymbolMetadata {
        name: "number".to_string(),
        is_terminal: true,
    });
    ...
    c.bench_function("parser_expression", |b| {
        b.iter(|| {
            ...
            let tokens = vec![
                ...
                Token { symbol: SymbolId(3), text: "*".to_string(), start: 10, end: 11 },
                Token { symbol: SymbolId(1), text: "789".to_string(), start: 12, end: 15 },
            ];
            let _ = parser.parse(black_box(&tokens));
        });
    });
