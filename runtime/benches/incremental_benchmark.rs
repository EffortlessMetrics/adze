...
use rust_sitter::{
    ...
    glr_lexer::{TokenWithPosition, GLRLexer},
};
...
fn benchmark_incremental_parsing(c: &mut Criterion) {
    ...
    for size in [10, 50, 100].iter() {
        ...
        let edit_pos = size / 2;
        let modified_input = generate_input(edit_pos) + " + 999 + " + &(edit_pos+1..*size).map(|i| i.to_string()).collect::<Vec<_>>().join(" + ");
        let modified_lexer = GLRLexer::new(&grammar, &modified_input).expect("Failed to create lexer");
        ...
        let edit = Edit {
            ...
            start_position: Position { line: 0, column: old_token.byte_offset },
            old_end_position: Position { line: 0, column: old_token.byte_offset + old_token.byte_length },
            new_end_position: Position { line: 0, column: new_token.byte_offset + new_token.byte_length },
        };
        ...
        if *size <= 100 {
            ...
            let tree = incremental.parse_incremental(&tokens, &[], None)
                .expect("Initial parse failed");
