...
use rust_sitter_ir::{Grammar, TokenPattern, SymbolId, Rule};
...
fn create_benchmark_grammar() -> (Grammar, ParseTable) {
    ...
    grammar.tokens.insert(id_symbol, rust_sitter_ir::Token {
        name: "identifier".to_string(),
        pattern: TokenPattern::Regex(r"[a-zA-Z_][a-zA-Z0-9_]*".to_string()),
        fragile: false,
    });
    
    grammar.tokens.insert(number_symbol, rust_sitter_ir::Token {
        name: "number".to_string(),
        pattern: TokenPattern::Regex(r"\d+".to_string()),
        fragile: false,
    });
    
    grammar.tokens.insert(whitespace_symbol, rust_sitter_ir::Token {
        name: "whitespace".to_string(),
        pattern: TokenPattern::Regex(r"\s+".to_string()),
        fragile: false,
    });
    
    grammar.tokens.insert(keyword_symbol, rust_sitter_ir::Token {
        name: "function".to_string(),
        pattern: TokenPattern::String("function".to_string()),
        fragile: false,
    });
    
    grammar.tokens.insert(operator_symbol, rust_sitter_ir::Token {
        name: "plus".to_string(),
...
fn generate_test_input(size_kb: usize) -> String {
    ...
    let line = "function calculate_value_12345(param_x, param_y) { return param_x + param_y + 42; }\n";
...
fn bench_standard_lexer(c: &mut Criterion) {
    ...
    let patterns: Vec<_> = grammar.tokens.iter()
...
fn bench_parallel_parser(c: &mut Criterion) {
    ...
    for size_kb in [100, 500, 1000].iter() {
        ...
        group.bench_with_input(
            ...
            &input,
            |b, input| {
                let parser = ParallelParser::new(grammar.clone(), parse_table.clone(), config.clone());
                b.iter(|| {
                    let result = parser.parse(input);
                    black_box(result)
                });
