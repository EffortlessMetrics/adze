...
use crate::pure_parser::{Parser, ParsedNode, ParseResult, TSLanguage, Point};
use std::ops::Range;
    ...
    pub fn parse(&mut self, source: &str, old_tree: Option<&Tree>) -> ParseResult {
        ...
        if let Some(root) = &result.root {
            if let Some(language) = self.parser.language() {
                self.previous_tree = Some(Tree::new(
                    root.clone(),
                    language,
                    source.as_bytes()
                ));
...
mod tests {
    ...
    fn test_tree_edit() {
        ...
        let language = unsafe { &crate::pure_parser::TSLanguage {
            version: 15,
            symbol_count: 10,
            token_count: 5,
            state_count: 20,
            ...
            lex_fn: None,
            external_scanner: crate::pure_parser::ExternalScanner { scan: None },
        }};
