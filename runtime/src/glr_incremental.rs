...
/// Tokenize source code using the grammar's token definitions.
fn tokenize(source: &[u8], grammar: &Grammar) -> Vec<GLRToken> {
    ...
    let mut glr_tokens = Vec::new();
    let mut position = 0;
    while position < source.len() {
        if let Some(token) = lexer.next_token(source, position) {
            if token.symbol == SymbolId(0) { // EOF
                break;
            }

            if !grammar.extras.contains(&token.symbol) {
                glr_tokens.push(GLRToken {
                    symbol: token.symbol,
                    text: token.text,
            ...
            position = token.end;
            if token.start == token.end { // Avoid infinite loops on zero-length tokens
                position += 1;
            }
        } else {
            // Unrecognized token, advance by one byte
            position += 1;
        }
    }
    glr_tokens
...
pub fn reparse(
    ...
    #[cfg(feature = "incremental_glr")]
    {
        ...
        let mut parser =
            IncrementalGLRParser::new_with_forest(grammar.clone(), table.clone(), Some(old_forest.clone()));
        ...
        let old_tokens = tokenize(&old_source, grammar);
        ...
        let mut edited_tokens = tokenize(new_text, grammar);
...
impl IncrementalGLRParser {
    ...
    fn splice_forests(
        ...
        let byte_start = all_children.first().map(|n| n.byte_range.start).unwrap_or(0);
