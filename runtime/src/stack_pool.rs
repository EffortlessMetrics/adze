...
pub struct StackPool<T: Clone> {
    ...
    /// Uses LIFO (stack) behavior for better cache locality of recently released stacks.
...
impl<T: Clone> StackPool<T> {
    ...
    pub fn acquire(&self) -> Vec<T> {
        ...
        // LIFO: pop the most recently used stack (better cache locality)
    ...
    pub fn acquire_with_capacity(&self, capacity: usize) -> Vec<T> {
        ...
        if let Some(pos) = pool.iter().position(|s| s.capacity() >= capacity) {
            // O(1) removal using swap_remove, since order doesn't matter
