...
use crate::incremental_v3::{Tree, Subtree, SubtreePool};
...
impl ParallelParser {
    ...
    fn parse_chunk(&self, chunk_id: usize, chunk: ParseChunk) -> ChunkResult {
        ...
        let parse_input = match &chunk.boundary {
            ...
            ChunkBoundary::Dirty { lookbehind, lookahead } => {
                // Include context for proper parsing
                let mut combined = Vec::with_capacity(
                    lookbehind.len() + chunk.content.len() + lookahead.len()
                );
    ...
    fn extract_subtrees(&self, tree: ParseNode, offset: usize) -> Vec<Subtree> {
        ...
        let subtree = Subtree {
            ...
            children: tree.children.into_iter()
    ...
    fn merge_chunk_results(&self, mut results: Vec<ChunkResult>, input: &[u8]) -> Result<ParseNode> {
    ...
    fn build_tree_from_subtrees(&self, subtrees: Vec<Subtree>, input: &[u8]) -> Result<ParseNode> {
        ...
        Ok(ParseNode {
            ...
            children: subtrees.into_iter().map(|st| self.subtree_to_node(st)).collect(),
            start_byte: 0,
            end_byte: input.len(),
            field_name: None,
        })
    ...
    fn subtree_to_node(&self, subtree: Subtree) -> ParseNode {
        ParseNode {
            ...
            children: subtree.children.into_iter().map(|st| self.subtree_to_node(st)).collect(),
