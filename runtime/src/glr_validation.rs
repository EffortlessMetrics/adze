...
impl GLRGrammarValidator {
    ...
    fn build_symbol_names(&mut self, grammar: &Grammar) {
        ...
        for external in &grammar.externals {
            self.symbol_names.insert(external.symbol_id, external.name.clone());
        }
    }

    fn get_symbol_name(&self, id: SymbolId) -> String {
        self.symbol_names.get(&id)
            .cloned()
    ...
    fn validate_basic_structure(&mut self, grammar: &Grammar) {
        ...
        if grammar.rules.is_empty() {
            self.errors.push(ValidationError {
                ...
                suggestion: Some("Add at least one rule to define the grammar structure".to_string()),
                related: vec![],
            });
        }
        
        // Check for start symbol
        let has_explicit_start = grammar.rules.keys()
            .any(|id| self.get_symbol_name(*id).starts_with("_start") || 
                      self.get_symbol_name(*id) == "start");
        
        if !has_explicit_start && !grammar.rules.is_empty() {
            let first_rule = grammar.rules.keys().next().unwrap();
            self.warnings.push(ValidationWarning {
                message: format!("No explicit start rule found, using '{}' as start symbol", 
                               self.get_symbol_name(*first_rule)),
                location: "Grammar root".to_string(),
                suggestion: Some("Consider adding an explicit '_start' rule for clarity".to_string()),
            });
        }
    }
        ...
        for (rule_id, rule) in &grammar.rules {
            for (pos, symbol) in rule.rhs.iter().enumerate() {
                ...
                if !defined_symbols.contains(&symbol_id) {
                    ...
                    let similar = self.find_similar_symbols(&symbol_id, &defined_symbols, grammar);
                    for sim in similar {
        ...
        for (symbol, count) in rule_counts {
            if count > 1 {
                self.errors.push(ValidationError {
                    ...
                    message: format!("Symbol '{}' has {} rule definitions", 
                                   self.get_symbol_name(symbol), count),
                    location: ErrorLocation {
                        symbol: Some(symbol),
                        rule_index: None,
                        position: None,
                    ...
                    suggestion: Some("Each symbol should have exactly one rule definition".to_string()),
                    related: vec![],
                });
            }
        }
    ...
    fn find_similar_symbols(&self, target: &SymbolId, defined: &HashSet<SymbolId>, _grammar: &Grammar) -> Vec<SymbolId> {
    ...
    fn validate_reachability(&mut self, reachable: &HashSet<SymbolId>, grammar: &Grammar) {
        ...
        for (symbol, _token) in &grammar.tokens {
            if !reachable.contains(symbol) {
                self.warnings.push(ValidationWarning {
                    message: format!("Token '{}' is defined but never used", 
                                   self.get_symbol_name(*symbol)),
    ...
    fn find_non_productive_cycle(&self, start: SymbolId, grammar: &Grammar, path: &mut Vec<SymbolId>, visited: &mut HashSet<SymbolId>) -> bool {
    ...
    fn check_left_recursion(&mut self, grammar: &Grammar, stats: &mut GrammarStats) {
        for (symbol, rule) in &grammar.rules {
            ...
            if self.has_indirect_left_recursion(*symbol, grammar) {
                stats.has_left_recursion = true;
                self.warnings.push(ValidationWarning {
                    message: format!("Indirect left recursion detected for '{}'", 
                                   self.get_symbol_name(*symbol)),
                    location: format!("Starting from rule for '{}'", self.get_symbol_name(*symbol)),
                    suggestion: Some("GLR parsing can handle left recursion efficiently".to_string()),
                });
            }
        }
    }
    ...
    fn can_derive_left(&self, from: SymbolId, target: SymbolId, grammar: &Grammar, visited: &mut HashSet<SymbolId>) -> bool {
            ...
            match &token.pattern {
                TokenPattern::String(s) if s.is_empty() => {
                    self.errors.push(ValidationError {
                        ...
                        message: format!("Token '{}' has empty string pattern", 
                                       self.get_symbol_name(*symbol)),
                        location: ErrorLocation {
                            symbol: Some(*symbol),
                            rule_index: None,
                ...
                TokenPattern::Regex(r) => {
                    ...
                    match regex::Regex::new(r) {
                        Err(e) => {
                            self.errors.push(ValidationError {
                                kind: ErrorKind::InvalidToken,
                                message: format!("Invalid regex pattern for token '{}'", 
                                               self.get_symbol_name(*symbol)),
                                location: ErrorLocation {
                                    symbol: Some(*symbol),
                                    rule_index: None,
                                    position: None,
                                    description: format!("Token '{}' pattern: {}", 
                                                      self.get_symbol_name(*symbol), r),
                                },
                        ...
                        Ok(re) => {
                            ...
                            if r.contains("*+") || r.contains("++") || r.contains("?+") {
                                self.warnings.push(ValidationWarning {
                                    message: format!("Possessive quantifiers in token '{}'", 
                                                   self.get_symbol_name(*symbol)),
                                    location: format!("Pattern: {}", r),
                                    suggestion: Some("Consider if possessive quantifiers are intended".to_string()),
                                });
                            }
                            
                            if !r.starts_with('^') && re.find_at("test", 1).is_some() {
                                self.warnings.push(ValidationWarning {
                                    message: format!("Token '{}' pattern may match at non-boundary", 
                                                   self.get_symbol_name(*symbol)),
                                    location: format!("Pattern: {}", r),
                                    suggestion: Some("Consider anchoring with ^ for consistent tokenization".to_string()),
                                });
                            }
                        }
                    }
        ...
        for i in 0..patterns.len() {
            for j in i + 1..patterns.len() {
                if self.patterns_overlap(&patterns[i].1, &patterns[j].1) {
                    self.warnings.push(ValidationWarning {
                        message: format!("Tokens '{}' and '{}' have overlapping patterns", 
                                       self.get_symbol_name(patterns[i].0),
                                       self.get_symbol_name(patterns[j].0)),
                        location: "Token definitions".to_string(),
                        suggestion: Some("Ensure token precedence is correctly defined".to_string()),
                    });
                }
            }
        }
            ...
            if precs.len() > 1 {
                ...
                if levels.len() > 1 {
                    self.errors.push(ValidationError {
                        ...
                        message: format!("Symbol '{}' has conflicting precedence levels", 
                                       self.get_symbol_name(symbol)),
                        location: ErrorLocation {
                            symbol: Some(symbol),
                            rule_index: None,
                        ...
                        suggestion: Some("Each symbol should have at most one precedence level".to_string()),
                        related: precs.iter().map(|(level, loc)| RelatedInfo {
                            location: loc.clone(),
    ...
    fn generate_suggestions(&self, grammar: &Grammar, stats: &GrammarStats) -> Vec<String> {
        ...
        if stats.has_left_recursion {
            suggestions.push("Grammar contains left recursion - GLR parsing is recommended".to_string());
        }
        
        if stats.requires_glr {
            suggestions.push("Grammar has ambiguities that require GLR parsing for correct handling".to_string());
        }
        ...
        if grammar.precedences.is_empty() && stats.requires_glr {
            suggestions.push("Consider adding precedence declarations to resolve ambiguities".to_string());
        }
        
        if grammar.fields.is_empty() && grammar.rules.len() > 5 {
            suggestions.push("Consider adding field names to rules for better AST access".to_string());
        }
...
mod tests {
    ...
    fn test_empty_grammar_error() {
        ...
        assert!(result.errors.iter().any(|e| e.kind == ErrorKind::EmptyGrammar));
    }
    ...
    fn test_undefined_symbol_with_suggestions() {
        ...
        // Define a token
        grammar.tokens.insert(SymbolId(1), rust_sitter_ir::Token {
        ...
        assert!(!result.is_valid);
        let undefined_error = result.errors.iter()
    ...
    fn test_left_recursion_detection() {
        ...
        grammar.tokens.insert(number_id, rust_sitter_ir::Token {
            name: "number".to_string(),
            pattern: TokenPattern::Regex(r"\d+".to_string()),
            fragile: false,
        });
        ...
        assert!(result.warnings.iter().any(|w| 
            w.message.contains("left recursion")));
    }
}