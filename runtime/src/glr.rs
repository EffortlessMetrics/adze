...
impl GLRStack {
    ...
    fn nodes_equal(nodes: &[ParseNode]) -> bool {
        ...
        let first = &nodes[0];
        nodes[1..].iter().all(|node| {
            node.symbol == first.symbol &&
            node.children.len() == first.children.len()
        })
    }
...
impl GLRParser {
    ...
    pub fn process_token(&mut self, token_symbol: SymbolId, get_action: impl Fn(StateId, SymbolId) -> Action) -> Result<()> {
