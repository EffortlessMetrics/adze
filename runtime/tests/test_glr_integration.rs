...
fn convert_to_query_subtree(subtree: &Arc<Subtree>) -> glr_query::Subtree {
    glr_query::Subtree {
        ...
        children: subtree.children.iter()
...
fn test_full_glr_pipeline() {
    ...
    assert!(validation_result.errors.is_empty(), "Grammar validation failed: {:?}", validation_result.errors);
...
fn test_precedence_and_associativity() {
    ...
    for (input, description) in test_cases {
        ...
        assert!(tree.is_some(), "Failed to parse: {} ({})", input, description);
...
fn test_error_recovery() {
    ...
    for (input, description) in invalid_inputs {
        ...
        if let Ok(mut lexer) = lexer_result {
            let tokens = lexer.tokenize_all();
            println!("Attempting to parse invalid input: {} ({})", input, description);
            println!("Tokens: {:?}", tokens.iter().map(|t| t.symbol_id).collect::<Vec<_>>());
...
fn test_complex_query_patterns() {
    ...
    for (query_str, description, _expected_count) in query_tests {
        ...
        println!("✓ Query '{}' ({}): found {} matches", query_str, description, matches.len());
