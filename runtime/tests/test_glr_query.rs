...
fn create_test_grammar() -> Grammar {
    ...
    grammar.rule_names.insert(add_expr_id, "add_expression".to_string());
    grammar.rule_names.insert(mul_expr_id, "mul_expression".to_string());
    grammar.rule_names.insert(paren_expr_id, "paren_expression".to_string());
...
fn test_query_with_predicates() {
    ...
    let query_parser = QueryParser::new(&grammar, 
...
fn test_query_parser_errors() {
    ...
    for (query_str, expected_error) in test_cases {
        ...
        assert!(error_str.contains(expected_error), 
            "Expected {} error for query: {}, got: {}", expected_error, query_str, error_str);
    }
}
