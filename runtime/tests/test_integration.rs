...
fn create_python_like_grammar() -> Grammar {
    ...
    grammar.tokens.insert(equals, Token {
...
fn test_full_parsing_pipeline() {
    ...
    let mut parser = Parser::new(grammar.clone(), parse_table)
...
fn test_incremental_parsing_pipeline() {
    ...
    let tree_v1 = incremental_parser.parse(input_v1, None, &[]).unwrap_or_else(|_| {
        ParseNode {
            symbol: SymbolId(200),
            children: vec![],
            start_byte: 0,
    ...
    let tree_v2 = incremental_parser.parse(input_v2, Some(&tree_v1), &edits).unwrap_or_else(|_| {
        ParseNode {
            symbol: SymbolId(200),
            children: vec![],
            start_byte: 0,
    ...
    match compile_query(query_str, &grammar) {
        Ok(query) => {
            ...
            for match_ in matches {
                println!("Match: pattern={}, captures={}", 
