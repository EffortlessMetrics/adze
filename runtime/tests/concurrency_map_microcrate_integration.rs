...
fn runtime_reexport_matches_owner_module_behavior() {
    ...
    for concurrency in 0..=32 {
        ...
        owner.sort_unstable();
        assert_eq!(runtime, owner, "concurrency={concurrency}");
    }
}

#[test]
fn runtime_reexport_stays_type_compatible_with_owner_module() {
    type TransformFn = fn(i32) -> i32;
    type BoundedMapFn = fn(Vec<i32>, usize, TransformFn) -> Vec<i32>;

    fn accepts_owner_fn(f: BoundedMapFn) -> BoundedMapFn {
        f
    }

    let returned = accepts_owner_fn(runtime_bounded_parallel_map::<i32, i32, TransformFn>);
