...
mod pure_rust_real_grammar_tests {
    ...
    fn test_incremental_parsing_real_world() {
        ...
        let edit = Edit {
            ...
            start_point: Point { row: 0, column: edit_start },
            old_end_point: Point { row: 0, column: edit_start },
            new_end_point: Point { row: 0, column: edit_start + 26 },
        };
    ...
    fn create_json_language() -> &'static TSLanguage {
        use rust_sitter::pure_parser::{TSParseAction, TSParseActionType, ExternalScanner};
        
        static PARSE_TABLE: &[u16] = &[0; 1000];
        static SMALL_PARSE_TABLE: &[u16] = &[0; 100];
        static PARSE_ACTIONS: &[TSParseAction] = &[
            TSParseAction {
...
fn test_build_and_use_real_grammar() {
    ...
    match build_parser_from_grammar_js(grammar_path, options) {
        Ok(result) => {
            ...
            println!("NODE_TYPES.json length: {} bytes", result.node_types_json.len());
