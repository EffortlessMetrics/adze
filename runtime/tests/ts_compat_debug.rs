...
fn debug_parse_flow() {
    ...
    for (i, rule) in lang.table.rules.iter().enumerate() {
        let lhs_name = lang.grammar.rule_names.get(&rule.lhs)
            .map(|s| s.as_str())
            .unwrap_or("?");
        println!("Rule {}: {} (id {:?}) -> {} symbols", 
    ...
    for (sym_id, actions) in lang.table.action_table[0].iter().enumerate() {
        if !actions.is_empty() {
            let sym_name = lang.grammar.rule_names.get(&rust_sitter::rust_sitter_ir::SymbolId(sym_id as u16))
                .map(|s| s.as_str())
                .unwrap_or("?");
    ...
    for state in 0..lang.table.state_count {
        for sym in 0..lang.table.symbol_count {
            ...
            if goto_state.0 != 0 {
                let sym_name = lang.grammar.rule_names.get(&rust_sitter::rust_sitter_ir::SymbolId(sym as u16))
                    .map(|s| s.as_str())
                    .unwrap_or("?");
                println!("  State {} + Symbol {} ({}) -> State {}", 
