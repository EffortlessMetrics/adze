...
mod incremental_reuse_tests {
    use rust_sitter::glr_incremental::{
        ...
        IncrementalGLRParser, GLREdit, GLRToken, Edit
    };
    ...
    use rust_sitter_ir::{Grammar, SymbolId, Symbol, Token, TokenPattern, Rule, 
                           Associativity, ProductionId, PrecedenceKind};
    use std::sync::Arc;
    ...
    fn test_simple_edit_reuses_subtrees() {
        ...
        for token in &initial_glr_tokens {
            println!("  Token: symbol={:?}, text={:?}, start={}, end={}", 
                     token.symbol, 
    ...
    fn test_multiple_edits_reuse() {
        ...
        if reuse_count == 0 {
            ...
            println!("Successfully reused {} subtrees with multiple edits", reuse_count);
        }
