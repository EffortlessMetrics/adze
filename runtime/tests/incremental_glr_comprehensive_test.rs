...
mod comprehensive_incremental_tests {
    ...
    use rust_sitter_ir::{Grammar, SymbolId, Symbol, Token, TokenPattern, Rule, 
                           ProductionId};
    ...
    fn create_arithmetic_grammar() -> Grammar {
        ...
        grammar.tokens.insert(star_id, Token {
            name: "STAR".to_string(),
            pattern: TokenPattern::String("*".to_string()),
            fragile: false,
        });
    ...
    fn test_multiple_edits() {
        ...
        reset_reuse_counter();
        let new_forest = parser.parse_incremental(&new_glr_tokens, &[edit1, edit2]).unwrap();
        ...
        if reuse_count == 0 {
            ...
            println!("✅ Multiple edits test passed: {} subtrees reused", reuse_count);
        }
        assert!(new_forest.alternatives.len() > 0, "Multiple edits should produce valid parse");
    }
    ...
    fn test_edit_at_beginning() {
        ...
        let reuse_count = get_reuse_count();
        assert!(new_forest.alternatives.len() > 0, "Edit at beginning should produce valid parse");
        println!("✅ Edit at beginning test passed: {} subtrees reused", reuse_count);
    }
    ...
    fn test_edit_at_end() {
        ...
        let reuse_count = get_reuse_count();
        assert!(new_forest.alternatives.len() > 0, "Edit at end should produce valid parse");
        println!("✅ Edit at end test passed: {} subtrees reused", reuse_count);
    }
    ...
    fn test_large_file_performance() {
        ...
        for i in 0..500 {  // Make it larger to see real performance difference
            if i > 0 {
        ...
        if let Some(digit_pos) = new_expr[edit_pos..].chars().position(|c| c.is_ascii_digit()) {
        ...
        let speedup = initial_parse_time.as_nanos() as f64 / incremental_parse_time.as_nanos().max(1) as f64;
    ...
    fn test_insertion() {
        ...
        assert!(new_forest.alternatives.len() > 0, "Insertion should produce valid parse");
        println!("✅ Insertion test passed: {} subtrees reused", get_reuse_count());
    }
    ...
    fn test_deletion() {
        ...
        assert!(new_forest.alternatives.len() > 0, "Deletion should produce valid parse");
        println!("✅ Deletion test passed: {} subtrees reused", get_reuse_count());
    }
    ...
    fn test_expansion() {
        ...
        assert!(new_forest.alternatives.len() > 0, "Expansion should produce valid parse");
        println!("✅ Expansion test passed: {} subtrees reused", get_reuse_count());
    }
    ...
    fn test_gss_snapshots() {
        ...
        let mut expr = String::new();
        for i in 0..150 {  // More than 100 tokens to trigger snapshots
            if i > 0 {
                expr.push('+');
            }
            expr.push_str(&i.to_string());
        ...
        let reuse_count = get_reuse_count();
        assert!(reuse_count > 0, "GSS snapshots should enable significant reuse");
        assert!(new_forest.alternatives.len() > 0, "Should produce valid parse with snapshots");
        println!("✅ GSS snapshot test passed: {} subtrees reused", reuse_count);
    }
}