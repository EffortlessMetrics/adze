...
fn create_arithmetic_grammar() -> Grammar {
    ...
    grammar.rules.insert(SymbolId(20), Rule {
        lhs: e_id,
        rhs: vec![Symbol::NonTerminal(e_id), Symbol::Terminal(plus_id), Symbol::NonTerminal(e_id)],
    ...
    grammar.rules.insert(SymbolId(21), Rule {
        lhs: e_id,
        rhs: vec![Symbol::NonTerminal(e_id), Symbol::Terminal(times_id), Symbol::NonTerminal(e_id)],
...
fn test_simple_fork_merge() {
    ...
    assert!(stack_counts.iter().any(|&c| c > 1), 
            "Expected multiple stacks during parsing of ambiguous input, but got {:?}", stack_counts);
...
fn test_complex_ambiguity() {
    ...
    let result = parser.finish();
    assert!(result.is_ok(), "Parser should handle highly ambiguous input");
    
    // For "aaaa", we expect many possible parse trees
    assert!(max_stacks > 2, "Expected many stacks for highly ambiguous input, but got max {}", max_stacks);
...
fn test_precedence_disambiguation() {
    ...
    println!("\n=== Testing precedence disambiguation with input '{}' ===", input);
    ...
    assert_eq!(root_op, Some(SymbolId(2)), "Root should be addition operator");
...
fn test_merge_identical_stacks() {
    ...
    assert!(max_stacks < 20, "Stack count suggests merging may not be working properly: {}", max_stacks);
