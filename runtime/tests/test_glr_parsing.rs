...
fn create_ambiguous_grammar() -> Grammar {
    ...
    grammar.tokens.insert(mult, Token {
        name: "mult".to_string(),
        pattern: TokenPattern::String("*".to_string()),
        fragile: false,
    });
...
fn create_conflicting_parse_table() -> ParseTable {
    ...
    action_table[4][3] = Action::Fork(vec![
        Action::Shift(StateId(6)),
...
fn test_glr_parser_coordinator() {
    ...
    glr_parser.stack.stacks[0].state_stack.push(rust_sitter::glr::ParserState {
        state: StateId(4),
        symbol: None,
        position: 5,
    });
