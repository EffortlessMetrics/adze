...
use rust_sitter::query::{Query, Highlighter, Theme};
...
fn create_simple_grammar() -> Grammar {
    ...
    // Add tokens
    grammar.tokens.insert(keyword_if, Token {
        name: "if".to_string(),
        pattern: TokenPattern::String("if".to_string()),
        fragile: false,
    });
    
    grammar.tokens.insert(keyword_else, Token {
        name: "else".to_string(),
    ...
    grammar.tokens.insert(lbrace, Token {
        name: "{".to_string(),
        pattern: TokenPattern::String("{".to_string()),
        fragile: false,
    });
    
    grammar.tokens.insert(rbrace, Token {
        name: "}".to_string(),
        pattern: TokenPattern::String("}".to_string()),
        fragile: false,
    });
    ...
    grammar.rule_names.insert(if_statement, "if_statement".to_string());
...
fn test_highlight_overlap_removal() {
    ...
    assert!(highlights.iter().all(|h| h.highlight == "variable" || h.highlight == "expression"));
}