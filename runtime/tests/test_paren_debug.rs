...
fn test_paren_debug() {
    ...
    println!("\nParse table built with {} states", parse_table.state_count);
    ...
    for (input, desc) in test_cases {
        ...
        println!("Tokens: {:?}", tokens.iter().map(|t| (t.symbol_id.0, &t.text)).collect::<Vec<_>>());
        
        // Process tokens with debug output
        for (i, token) in tokens.iter().enumerate() {
            println!("\n--- Processing token {}: {:?} ---", i, (token.symbol_id.0, &token.text));
