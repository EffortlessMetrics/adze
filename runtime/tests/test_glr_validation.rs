...
use glr_validation::{GLRGrammarValidator, ErrorKind};
...
fn test_comprehensive_validation() {
    ...
    assert!(result.warnings.iter().any(|w| w.message.contains("left recursion")));
...
fn test_undefined_symbol_with_typo() {
    ...
    grammar.tokens.insert(identifier_id, Token {
        name: "identifier".to_string(),
        pattern: TokenPattern::Regex(r"[a-zA-Z_]\w*".to_string()),
        fragile: false,
    });
    grammar.rule_names.insert(identifier_id, "identifier".to_string());
    ...
    grammar.rule_names.insert(expr_id, "expr".to_string());
    grammar.rule_names.insert(identifer_typo_id, "identifer".to_string()); // typo
    ...
    assert!(undefined_error.related.iter().any(|r| r.message.contains("identifier")));
...
fn test_non_productive_cycle_detection() {
    ...
    assert!(!result.is_valid);
    assert!(result.errors.iter().any(|e| e.kind == ErrorKind::NonProductiveSymbol));
    
    // Should identify all symbols in the cycle
    let non_prod_errors: Vec<_> = result.errors.iter()
...
fn test_ambiguous_grammar_detection() {
    ...
    assert!(result.warnings.iter().any(|w| w.message.contains("ambiguity")));
...
fn test_token_validation() {
    ...
    grammar.tokens.insert(bad_regex_id, Token {
        name: "bad_regex".to_string(),
        pattern: TokenPattern::Regex(r"[a-".to_string()), // Invalid regex
        fragile: false,
    });
    grammar.rule_names.insert(bad_regex_id, "bad_regex".to_string());
    
    // Empty string token
    let empty_id = SymbolId(2);
    grammar.tokens.insert(empty_id, Token {
    ...
    grammar.tokens.insert(word1_id, Token {
    ...
    grammar.tokens.insert(word2_id, Token {
    ...
    assert!(result.errors.iter().any(|e| 
        e.kind == ErrorKind::InvalidToken && e.message.contains("regex")));
    assert!(result.errors.iter().any(|e| 
        e.kind == ErrorKind::InvalidToken && e.message.contains("empty")));
    
    // Should warn about overlapping tokens
    assert!(result.warnings.iter().any(|w| w.message.contains("overlapping")));
}
...
fn test_helpful_error_formatting() {
    ...
    grammar.rule_names.insert(undefined_id, "missing_token".to_string());
...
fn test_unreachable_symbol_warning() {
    ...
    grammar.tokens.insert(used_token, Token {
        name: "used".to_string(),
    ...
    grammar.tokens.insert(unused_token, Token {
        name: "unused".to_string(),
        pattern: TokenPattern::String("unused".to_string()),
        fragile: false,
    });
    grammar.rule_names.insert(unused_token, "unused".to_string());
    ...
    grammar.rule_names.insert(unreachable_rule, "unreachable_rule".to_string());
    ...
    assert!(result.warnings.iter().any(|w| 
        w.message.contains("unused") && w.message.contains("never used")));
    assert!(result.warnings.iter().any(|w| 
        w.message.contains("unreachable_rule") && w.message.contains("not reachable")));
