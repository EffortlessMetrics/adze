...
fn test_complete_workflow() {
    ...
    assert!(result.errors.is_empty(), "Initial parse had errors: {:?}", result.errors);
    ...
    assert!(incremental_result.errors.is_empty(), "Incremental parse had errors");
    assert!(incremental_result.root.is_some(), "No incremental parse tree");
    
    // 6. Verify the edit was applied
    let root = incremental_result.root.as_ref().unwrap();
    assert!(find_node_with_text(root, edited_source, "43").is_some(), "Edit not reflected in tree");
}
...
fn test_error_recovery() {
    ...
    assert!(!result.errors.is_empty(), "No errors reported for invalid syntax");
...
fn test_cancellation() {
    ...
    assert!(!result.errors.is_empty() || result.root.is_none(), "Parse was not cancelled");
}
...
fn test_timeout() {
    ...
    // Should timeout
    assert!(!result.errors.is_empty() || result.root.is_none(), "Parse did not timeout");
}
...
fn test_external_scanner_integration() {
    ...
    assert_eq!(scanner.lock().unwrap().count, 0, "Scanner called unexpectedly");
}
