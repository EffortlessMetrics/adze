...
# We exclude the pattern where it's preceded by #[cfg(not(rust_sitter_unsafe_attrs))]
PROBLEMATIC_FILES=$(grep -r -n '^\s*#\s*\[no_mangle\]' --include="*.rs" . --exclude-dir=target 2>/dev/null | while IFS=: read -r file line content; do
    ...
    if [ "$line" -gt 1 ] 2>/dev/null; then
        prev_line=$((line - 1))
        if ! sed -n "${prev_line}p" "$file" | grep -q '^\s*#\s*\[cfg(not(rust_sitter_unsafe_attrs))\]'; then
...
if [ -n "$PROBLEMATIC_FILES" ]; then
    ...
    echo "  #[cfg(rust_sitter_unsafe_attrs)]"
    echo "  #[unsafe(no_mangle)]"
    echo "  #[cfg(not(rust_sitter_unsafe_attrs))]"
    echo "  #[no_mangle]"
    exit 1
else
