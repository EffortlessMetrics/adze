...
proptest! {
    ...
    // -----------------------------------------------------------------------
    #[test]
    fn grammar_postcard_roundtrip_empty(name in "[a-zA-Z][a-zA-Z0-9_]{0,15}") {
        let g = Grammar::new(name);
        let back = roundtrip_binary(&g);
        prop_assert_eq!(g, back);
    }
    ...
    // -----------------------------------------------------------------------
    // 4. Grammar with rules roundtrip (postcard)
    // -----------------------------------------------------------------------
    #[test]
    fn grammar_with_rules_postcard_roundtrip(
        name in "[a-zA-Z][a-zA-Z0-9_]{0,10}",
        rules in prop::collection::vec(arb_rule(), 1..6),
    ) {
        let mut g = Grammar::new(name);
    ...
    // -----------------------------------------------------------------------
    // 6. Grammar with externals roundtrip (postcard)
    // -----------------------------------------------------------------------
    #[test]
    fn grammar_with_externals_postcard_roundtrip(
        name in "[a-zA-Z][a-zA-Z0-9_]{0,10}",
        exts in prop::collection::vec(arb_external_token(), 1..5),
    ) {
        let mut g = Grammar::new(name);
    ...
    // -----------------------------------------------------------------------
    // 8. Grammar with precedences roundtrip (postcard)
    // -----------------------------------------------------------------------
    #[test]
    fn grammar_with_precedences_postcard_roundtrip(
        name in "[a-zA-Z][a-zA-Z0-9_]{0,10}",
        precs in prop::collection::vec(arb_precedence(), 1..5),
    ) {
        let mut g = Grammar::new(name);
    ...
    // -----------------------------------------------------------------------
    // 10. Serialization preserves all fields (postcard)
    // -----------------------------------------------------------------------
    #[test]
    fn serialization_preserves_all_fields_postcard(g in arb_full_grammar()) {
    ...
    // -----------------------------------------------------------------------
    #[test]
    fn deserialization_error_corrupt_postcard(
    ...
    // -----------------------------------------------------------------------
    // 14. Large grammar roundtrip (postcard)
    // -----------------------------------------------------------------------
    #[test]
    fn large_grammar_postcard_roundtrip(
        rules in prop::collection::vec(arb_rule(), 10..20),
        toks in prop::collection::vec((arb_symbol_id(), arb_token()), 5..10),
        precs in prop::collection::vec(arb_precedence(), 3..6),
        exts in prop::collection::vec(arb_external_token(), 2..5),
    ...
    // -----------------------------------------------------------------------
    #[test]
    fn binary_deterministic_grammar(g in arb_full_grammar()) {
    ...
    // -----------------------------------------------------------------------
    // 26. Double roundtrip idempotence (postcard)
    // -----------------------------------------------------------------------
    #[test]
    fn double_roundtrip_postcard(g in arb_full_grammar()) {
        let bytes1 = postcard::to_stdvec(&g).unwrap();
    ...
    // -----------------------------------------------------------------------
    #[test]
    fn truncated_postcard_fails(g in arb_full_grammar()) {
        let bytes = postcard::to_stdvec(&g).unwrap();
        if bytes.len() > 1 {
...
#[test]
fn builder_grammar_postcard_roundtrip() {
    let g = GrammarBuilder::new("arith")
        .token("NUMBER", r"\d+")
        .token("+", "+")
        .token("*", "*")
