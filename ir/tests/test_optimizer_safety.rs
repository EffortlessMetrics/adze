...
fn create_unit_rule_grammar() -> Grammar {
    ...
    grammar.tokens.insert(times, Token {
    ...
    grammar.rule_names.insert(source_file, "source_file".to_string());
...
fn test_unit_rule_elimination_preserves_semantics() {
    ...
    for (symbol_id, rules) in &optimized.rules {
        let name = optimized.rule_names.get(symbol_id)
            .map(|n| n.as_str())
            .unwrap_or("unknown");
    ...
    let source_file_id = optimized.find_symbol_by_name("source_file")
        .expect("source_file should exist");
    
    assert!(optimized.rules.contains_key(&source_file_id), "source_file should have rules");
    
    // Just verify the grammar is still functional
    assert!(optimized.rules.len() > 0, "Grammar should still have rules");
    assert!(optimized.tokens.len() > 0, "Grammar should still have tokens");
    
    // The optimization should have done something
    assert!(stats.total() > 0, "Some optimizations should have been performed");
}
...
fn test_optimizer_handles_left_recursion() {
    ...
    grammar.tokens.insert(item, Token {
        name: "item".to_string(),
        pattern: TokenPattern::Regex(r"\w+".to_string()),
        fragile: false,
    });
    
    grammar.tokens.insert(comma, Token {
    ...
    assert!(grammar.rules.len() > 0, "Grammar should still have rules after optimization");
}
...
fn test_optimizer_preserves_source_file_symbol() {
    ...
    grammar.rule_names.insert(source_file, "source_file".to_string());
    grammar.rule_names.insert(statement, "statement".to_string());
    ...
    let source_file_id = grammar.find_symbol_by_name("source_file")
        .expect("source_file should still exist after optimization");
    
    assert!(grammar.rules.contains_key(&source_file_id), "source_file should not be removed");
    assert_eq!(
