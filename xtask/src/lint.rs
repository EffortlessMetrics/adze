...
use crate::debug_blocks::{self, CheckOptions};
...
pub fn lint(
    ...
    if !fast {
        debug_blocks::self_test().context("validator self-tests failed")?;
    }

    debug_blocks::run(CheckOptions {
        fix,
        changed_only,
        since,
        files: Vec::new(),
...
fn run_command(bin: &str, args: &[&str]) -> Result<()> {
    ...
    if !status.success() {
        bail!("{bin} {:?} failed with {status}", args);
    }
    Ok(())
}
