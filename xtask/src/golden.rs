...
pub fn generate_golden(sh: &Shell, grammar: Grammar, force: bool) -> Result<()> {
    ...
    println!("Generating golden files for {}...", style(grammar.name()).cyan());
    ...
    println!("{} Golden files generated successfully!", style("✓").green());
    Ok(())
}
...
fn generate_external_golden(sh: &Shell, grammar: &Grammar, output_dir: &Path) -> Result<()> {
    ...
    cmd!(sh, "npx tree-sitter generate").run()
...
pub fn diff_golden(sh: &Shell, grammar: Grammar, verbose: bool) -> Result<()> {
    ...
    for entry in WalkDir::new(&golden_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        if golden_content != current_content {
            ...
            println!("{} Matches: {}", style("✓").green(), relative_path.display());
        }
    }
...
pub fn update_golden(sh: &Shell, grammar: Grammar) -> Result<()> {
    ...
    for entry in WalkDir::new(&current_dir).into_iter().filter_map(|e| e.ok()) {
        if entry.file_type().is_file() {
            ...
            println!("{} Updated: {}", style("✓").green(), relative_path.display());
        }
    }
...
pub fn test_all_golden(sh: &Shell, verbose: bool) -> Result<()> {
    let grammars = vec![Grammar::Arithmetic, Grammar::Javascript, Grammar::Rust, Grammar::Python];
    let mut all_passed = true;
    
    for grammar in grammars {
        let golden_dir = golden_path(&grammar);
        if !golden_dir.exists() {
            println!("{} Skipping {} (no golden files)", style("→").yellow(), grammar.name());
            continue;
        }
...
fn print_diff(golden: &str, current: &str) {
    ...
    for change in diff.iter_all_changes() {
        ...
        print!("{} {}", style_color.apply_to(sign), style_color.apply_to(change));
    }
    println!();
}
