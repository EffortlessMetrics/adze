...
pub(crate) fn run(check: bool) -> Result<()> {
    ...
    std::fs::create_dir_all(&target_dir).with_context(|| {
        format!(
            "failed to create badge target directory {}",
            target_dir.display()
        )
    })?;
    ...
    let committed_dir = workspace_root.join(BADGE_ENDPOINT_DIR);
    std::fs::create_dir_all(&committed_dir).with_context(|| {
        format!(
            "failed to create committed badge directory {}",
...
fn ripr_plus_badge(workspace_root: &Path) -> Result<ShieldsEndpointBadge> {
    ...
    let output = Command::new(&ripr_bin)
        ...
        .output()
        .with_context(|| format!("failed to run {ripr_bin} for ripr+ badge"))?;
...
#[allow(dead_code)]
fn _workspace_relative(path: &Path, workspace_root: &Path) -> PathBuf {
