...
module.exports = grammar({
  ...
  rules: {
    ...
    println!("✓ Generated parser for language: {}", build_result.grammar_name);
    ...
    for expr in test_expressions {
        ...
        if !result.errors.is_empty() {
            ...
            for error in &result.errors {
                println!("    - At position {}: expected symbols {:?}, found {}",
                    error.position,
                    error.expected,
                    error.found
                );
    ...
    println!("Parsing expression with ~{} tokens...", large_expr.split_whitespace().count());
