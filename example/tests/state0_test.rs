...
fn test_state0_has_token_actions() {
    ...
    assert!(
        state0_entries.len() >= 2,
        "State 0 should have at least one entry"
    );
    assert!(
        state0_entries.len() % 2 == 0,
        "State 0 entries should be pairs"
    );
    ...
    PRINT_ONCE.call_once(|| {
        ...
        for i in (0..state0_entries.len()).step_by(2) {
            ...
            if symbol < lang.token_count as u16 {
                println!(
                    "  - Token {} -> action 0x{:04x} (shift to state {})",
                    symbol,
                    action,
                    action & 0x7FFF
                );
...
fn test_parse_arithmetic_expression() {
    ...
    assert!(
        !parse_table_data.is_empty(),
        "Parse table should not be empty"
    );
