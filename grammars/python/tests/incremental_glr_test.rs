...
fn test_incremental_edit_simple() {
    ...
    parser.set_language(rust_sitter_python::get_language())
    ...
    let new_tree = parser.parse_with_old_tree(new_source.as_bytes(), Some(&old_tree), Some(&edit))
...
fn test_incremental_edit_into_ambiguity() {
    ...
    assert_eq!(new_tree.error_count(), 0, "Should parse expression without errors");
}
...
fn test_incremental_edit_multi_line() {
    ...
    assert_eq!(new_tree.error_count(), 0, "Should parse function without errors");
}
