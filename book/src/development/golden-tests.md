...
Golden tests in adze provide byte-for-byte compatibility verification between our pure-Rust parsers and the official Tree-sitter parsers. This ensures that adze produces identical parse trees for the same source code.

## What Are Golden Tests?

Golden tests (also called reference tests or regression tests) compare the output of adze parsers against "golden" reference files generated by the official Tree-sitter parsers. They validate:
...
Golden tests provide the foundation for reliable parser development and ensure adze maintains perfect compatibility with Tree-sitter's reference implementations.